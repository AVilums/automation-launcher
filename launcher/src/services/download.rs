...
impl DownloadManager {
    ...
    async fn download_with_resume(
        ...
        if let Some(token) = &self.token {
            ...
            // If it's a GitHub API URL, we must request the octet-stream to get the asset data
            if url.contains("api.github.com/repos/") && url.contains("/assets/") {
                request = request.header("Accept", "application/octet-stream");
            }
        }

        // Only resume if the file is at least 1 KB, otherwise just restart
        ...
        if !status.is_success() && status != reqwest::StatusCode::PARTIAL_CONTENT {
            return Err(LauncherError::Download(format!("Server returned error status: {}", status)));
        }
